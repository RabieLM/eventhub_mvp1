...
function mvcRouteUrl(path) {
    const entry = window.EVENTHUB_MVC_ENTRY || '';
    if (!entry) {
        return '';
    }
    ...
    const routeMap = {
        'api/events.php': '/api/events',
        'api/stats.php': '/api/stats',
        'events/register.php': '/events/register',
        'events/create.php': '/events/create',
    };
    ...
    return `${entry}${encodeURIComponent(route)}${rawQuery ? `&${rawQuery}` : ''}`;
}
...
async function fetchJson(url, options = {}) {
    ...
    try {
        ...
        throw new Error("Impossible de joindre l'API PHP. Ouvre le site via http://localhost/phpexam/eventhub_mvp/index.php ou /public/index.php et verifie qu'Apache est demarre.");
    }
